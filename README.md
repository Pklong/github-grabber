...
We'll use Node's [fs], [process], and [http], as well as the third-party package [nodemon] to build an application that fetches a user's starred repos from Github and writes them to a file.
...
We'll start with the [fs] module. We've a provided animals.txt file to supply data for our experiments. Make an empty file for our Node code in the same directory as animals.txt:

`touch animal_fun.js`

The Node runtime environment provides the `fs` module, but we'll need to `require` the module to use it in our file. 

To start, we'll read the animal file and `console.log` every entry.
...
**The order of arguments in these callbacks is very common in Node!**

Run the file with `node animal_fun.js`. Yipes, that doesn't look like a string at all! We're looking at a raw [Buffer] object. 

Neat, but since we know this is a text file and we'd like to make it human-readable, let's provide an additional argument to `readFile` that specifies the encoding...
...
Let's move on to [fs.writeFile]. There are a few differences: we provide the data that should be written to the file as the second argument, and our callback function is only provided the error object (again, if it exists).
